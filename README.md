...
> **本项目仅支持青龙面板（QingLong）。**
>
> 当前仓库仅针对青龙面板环境进行开发、适配和测试。**不支持云函数 / Serverless、独立 Docker、Windows、macOS、Linux 直接运行、GitHub Actions 等其他运行环境。**
>
> 如需在上述环境或其他非青龙环境中使用，请自行阅读并修改脚本，完成对应的定时任务、环境变量、账号凭证存储、通知及运行方式适配。本项目不提供这些环境的直接部署支持。

面向 **青龙面板（QingLong）** 的小米钱包每日任务脚本。

基于心扫码登录与小米钱包任务逻辑进行青龙适配，当前主要支持：
...
| `ZzzHe_小米钱包每日任务` | `1 6 * * *` | **首次创建时**默认每天 06:01 触发，先进行启动随机延迟，再串行执行所有账号 |

### 已有任务保护规则

`06:01` 只是仓库给 **新创建任务** 提供的默认时间。
...
同样，如果你已经在青龙里禁用了这个任务，仓库代码不会主动解除禁用状态。
...
> 如果青龙中已经有 `ZzzHe_小米钱包每日任务`，则保持已有 Cron，不自动修改；如果该任务已经禁用，也保持禁用，不自动启用。

实际核心任务开始时间还会受到 `A` 的启动随机延迟影响。

例如默认 `A=30` 且使用新任务默认时间：

```text
06:01 触发
→ 随机等待 9 ～ 30 分钟
→ 大约 06:10 ～ 06:31 开始 ck1
```

后续账号再按 `B` 串行间隔。

如果用户已经自行修改 Cron，则以上时间全部以用户当前 Cron 为准。
